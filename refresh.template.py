...
def _bazel():
    """The bazel binary path/name supplied to the refresh_compile_commands macro."""
...
def _get_source_file(compile_action):
    ...
    if not source_file_candidates:
        # Header-only / sourceless action (e.g. Bazel's parse_headers feature).
        # Caller is expected to drop these. We also disable the upstream
        # parse_headers feature via aquery, but be defensive here regardless.
        return None
...
_get_files.cpp_source_extensions = ('.cc', '.ccm', '.cpp', '.cppm', '.cxx', '.c++', '.C', '.CC', '.cp', '.CPP', '.C++', '.CXX', '.ii', '.ipp', '.hh', '.hpp', '.hxx')
...
def _get_cpp_command_for_files(compile_action):
    ...
    # aquery may omit the `value` field entirely when the env var is set to empty
    # (e.g. `build --action_env=SOME_ENV=` in .bazelrc); default to "" in that case.
    ...
    files_result = _get_files(compile_action)
    if files_result is None:
        return None
    source_files, header_files = files_result
...
def _convert_compile_commands(aquery_output):
    ...
    for output in outputs:
        if output is None: # Skipped sourceless action; see _get_files.
            continue
        source_files, header_files, compile_command_args = output
